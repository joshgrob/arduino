...
{  
  ...
  //setup power and GND pins.  We can't use the normal power and gnd outputs on the teensy since they are in use.
  pinMode(IMU_POWER_PIN, OUTPUT);
  pinMode(IMU_GND_PIN, OUTPUT);
  digitalWrite(IMU_POWER_PIN, HIGH);
  digitalWrite(IMU_GND_PIN, LOW);
...
{  
  ...
  //for first test just drive the motors based on gyro outputs calculated above.  Further experiments will use fusion algorithm
  drive_motor();
...
//for now we are driving the motor based on just the gyroscope rate of rotation values, and scaling the intensity of the 
//vibration appropriately.  Assume that we have the gyro pegged at angular rate resolution of 250 deg/sec
void drive_motor() {

  int max_gyro = 250;
  int motor_x_intensity = (int) abs((gx/max_gyro * PWM_MAX));
  int motor_y_intensity = (int) abs((gy/max_gyro * PWM_MAX));
  

  //write left or right PWM
  if (gx < 0.0) {
    analogWrite(MOTOR_RIGHT_PIN, 0);
    analogWrite(MOTOR_LEFT_PIN, motor_x_intensity); 
  } else if (gx > 0.0) {
    analogWrite(MOTOR_RIGHT_PIN, motor_x_intensity);
  ...
  //write up or down PWM
  if (gy < 0.0) {
    analogWrite(MOTOR_UP_PIN, 0);
    analogWrite(MOTOR_DOWN_PIN, motor_y_intensity);
  } else if (gy > 0.0) {
    analogWrite(MOTOR_UP_PIN, motor_y_intensity);
  ...
  //if(SerialDebug){
  //  Serial.print("Motor X PWM: ");Serial.print(motor_x_intensity);
  //  Serial.print("  Motor Y PWM: ");Serial.println(motor_y_intensity); 
  //}
}
